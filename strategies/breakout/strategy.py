...
class BreakoutRetestStrategy(BaseStrategy):
    ...
    Detects breakouts through key price levels, then enters
    when price retests the broken level.

    For bull breakouts:
    - Price breaks above level
    - Wait for retest (pullback to level)
    - Enter long with stop below level

    For bear breakouts:
    - Price breaks below level
    - Wait for retest (rally back to level)
    - Enter short with stop above level
    """
    ...
    def __init__(
        ...
        Args:
            ...
            config: Optional BreakoutConfig object
        """
        ...
        # Create detector
        self._detector = BreakoutDetector(self.bars, self.level_cols)

    def detect_triggers(self) -> List[TriggerEvent]:
        """Detect breakout trigger events."""
        return self._detector.detect()

    def detect_breakouts(self) -> List[tuple]:
        """
        Legacy interface for breakout detection.
    ...
    def find_entries(
        ...
        """
        Find confirmed entry signals from breakout triggers.

        Looks for price to retest the broken level within lookahead window.
        """
        signals = []
