...
sec_df = generate_second_data('2025-05-19', 30)
dt_idx = 'datetime'
day_idx = 'date'
...
utility_model.train()
# prob_util = utility_model.predict(utility_model.min_df)
# pred_util = (prob_util>=0.5).astype(int)
...
class UtilityStrategy(Strategy):
    ...
    def next(self):
        ...
        if self.data.datetime[-1].astype('M8[s]').item().time() >= time(15, 58):
            if self.position:
                self.position.close()
            return
        ...
        if pred_rev[0] == 1:
            print(prob_rev, contribs, sigmoid(sum(contribs[0])))
            exit()
